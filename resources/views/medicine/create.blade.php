...
@section('content')
<form action="{{ route('obat.tambah_obat.formulir') }}" method="POST" class="card-p-5x">
    {{--
        1. tag <form> attr action dan method
            method :
            - GET : form tujuan mencari data (search)
            - POST : form tujuan menambahkan/menghapus/mengubah

            action : route memproses data
            -arahkan route yg akan menangani proses ke dbnya
